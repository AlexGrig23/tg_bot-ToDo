...
class Categories:
    def __init__(self):
        self.categories = ["products", "entertainment", "health", "auto"]
        self.filename = filename

    def to_dict(self):
        return {f"{categor}": "" for categor in self.categories}
...
async def cat(update: Update, context: CallbackContext) -> None:
    new_categories_lst = Categories()
    categories_dict = new_categories_lst.to_dict()
    categories_string = [f"{key}\n" for key in categories_dict.keys()]
    await update.message.reply_text("".join(categories_string))
...
async def add_ex(update: Update, context: CallbackContext) -> None:
    ...
    with open(filename, "a") as f:
        f.write(new_line + ' ' + " | ".join(user_data) + " | " + ex + "\n")
...
async def add_in(update: Update, context: CallbackContext) -> None:
    ...
    with open(filename, "a") as f:
        f.write(new_line + " " + " | ".join(data_incoming) + " | " + inc + "\n")
...
async def list(update: Update, context: CallbackContext) -> None:
    with open(filename, "r") as f:
        lst = f.readlines()
        await update.message.reply_text(" ".join(lst))
...
async def remove(update: Update, context: CallbackContext) -> None:
    try:
        ...
        with open(filename, "w") as f:
            for line in lines:
                if not line.startswith(str(remove_idx)):
                    f.write(line)
